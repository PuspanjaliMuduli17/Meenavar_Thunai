...
class ChatProvider extends ChangeNotifier {
    ...
    try {
      ...
      final conversationHistory = _messages
          .where((msg) => 
              ...
              !msg.isLoading)
          .where((msg) => msg.id != userMessage.id) // Exclude current message
      ...
      // Remove loading message and handle error
      _removeMessage(loadingMessage.id);
      _error = e.toString();
  ...
  Future<void> _sendMessageInternal(String content) async {
    ...
    try {
      ...
      _removeMessage(loadingMessage.id);
      _error = e.toString();
      _addMessage(ChatMessage.error('Failed to regenerate response: ${e.toString()}'));
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  ...
  Map<String, int> get conversationStats {
    ...
    final errorMessages = _messages.where((msg) => msg.type == MessageType.error).length;
